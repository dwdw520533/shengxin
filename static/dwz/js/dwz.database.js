	...
	$.fn.extend({
		look_up: function(){
			return this.each(function(){
				var $this=$(this),$lookup = $(this).parent().find("a[lookupGroup]"), options = {mask:true, 
					width:$lookup.attr('width')||820, height:$lookup.attr('height')||400,
				    maxable:eval($lookup.attr("maxable") || "true"),
					minable:eval($lookup.attr("minable") || "true"),
					resizable:eval($lookup.attr("resizable") || "true")
				};
				$this.click(function(event){
					$this.val("");
					$this.blur();
					_lookup = $.extend(_lookup, {
						currentGroup: $lookup.attr("lookupGroup") || "",
						suffix: $lookup.attr("suffix") || "",
						$target: $lookup,
						pk: $lookup.attr("lookupPk") || "id"
					});
					var url = unescape($lookup.attr("href")).replaceTmById($(event.target).parents(".unitBox:first"));
					if (!url.isFinishedTm()) {
						alertMsg.error($lookup.attr("warn") || DWZ.msg("alertSelectMsg"));
						return false;
					}
					$.pdialog.open(url, "_blank", $lookup.attr("title") || $lookup.text(), options);
					return false;
				});
			});
		},
		lookup: function(){
			return this.each(function(){
				var $this = $(this), options = {mask:true, 
					...
					minable:eval($this.attr("minable") || "true"),
		...
		multLookup: function(){
			return this.each(function(){
				...
				$this.click(function(event){
					...
					if ($.isEmptyObject(args) && $this.attr("warn")) {
						alertMsg.error($this.attr("warn") || DWZ.msg("alertSelectMsg"));
						return false;
					}
					$.bringBack(args);
				...
				function _show(event){
					...
					$.ajax({
						...
						success: function(response){
							...
							if(html){
								$lis = $suggest.html('<ul>'+html+'</ul>').find("li");
								$lis.hoverClass("selected").click(function(){
									_select($(this));
								});
								if ($lis.size() == 1 && event.keyCode != DWZ.keyCode.BACKSPACE) {
