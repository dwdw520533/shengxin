...
var navTab = {
	...
	init: function(options){
		...
		this._moreBut.click(function(){
			if($this._moreBox.css("display") == "none"){
				$this._moreBox.show();
			}else{
				$this._moreBox.hide();
			}
	...
	closeAllTab: function(){
		...
		this._getPanels().filter(":gt(0)").find("iframe").attr("src","");
		this._getPanels().filter(":gt(0)").find("iframe").remove();
	...
	_reload: function($tab, flag, forwardUrl){
		flag = flag || $tab.data("reloadFlag");
		var url = forwardUrl || $tab.attr("url");
		if (flag && url) {
