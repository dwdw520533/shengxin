function initEnv() {
	...
	$("#leftside").jBar({minW:140, maxW:160});
...
function initLayout(){
    ...
    // for customservice
    if($("#picc").length > 0){
        $("#container .tabsPageContent").height(iContentH - 62).find("[layoutH]").layoutH();
    }
    else{
...
function initUI(_box){
	...
	if ($.fn.look_up) $("a[lookupGroup]", $p).parent().find(":input").each(function(i){
		$(this).look_up();
	});
	...
	//榜单click事件，页面查找带回出现翻页调整时
	$(".uniqb", $p).each(function(){
		$(this).click(function(event){
			var $this = $(this);
			var rel = $this.attr("sel");
			$("#searchForm").attr("action",rel)
			$("#searchForm").submit();			
			event.preventDefault();
		});
	});
	//给checkbox绑定事件
	$(".cbox",$p).change(function(event){
		$this = $(this)
		$("#chkbox").loadUrl($this.attr("ref")+"&"+$this.attr("pattr")+"="+$this.val())
		event.preventDefault();
	})
	
	//给部分下来选择框榜单change事件
	$(".chgClass",$p).change(function(event){
		if ($(this).val() == 1){
			$(".chgc",$p).each(function(){
	...
	//上传图片js控制		
	$(".uptainer",$p).each(function(){
		$(this).click(function(event){
			$str = '<tr>';
			$str += '<td><input class="required" name="tmpfile" size="60" type="file" /></td>';
			$str += '<td><a class="btnDel" onclick="del_cur_str(this)">删除</a></td></tr>';
			$("#upcontainer").append($str)
			event.preventDefault();
		})
	})
}
//删除新增的图片上传控件
function del_cur_str(a){
